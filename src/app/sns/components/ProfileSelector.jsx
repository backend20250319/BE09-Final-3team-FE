...
export default function ProfileSelector() {
  ...
  return (
    <div ref={containerRef} className={styles.profileInfo} style={{ margin: "8px 0 16px" }}>
      <div className={styles.profileHeader}>
        <div style={{ position: "relative", width: 280 }}>
          <button
          type="button"
          className={`${styles.profileSelectorButton} ${open ? styles.active : ""}`}
          onClick={() => setOpen((v) => !v)}
        ...
        >
          <Image src="/user/instagram.svg" alt="instagram" width={16} height={16} />
          {selectedInstagramProfile?.username ? (
            <>
              <span style={{ fontWeight: 600 }}>@{selectedInstagramProfile.username}</span>
        ...
        {open && (
          <div className={styles.profileSelector} style={{ left: 0, right: 0 }}>
            {loading ? (
              <div className={styles.loadingText}>프로필 로딩 중...</div>
            ) : error ? (
              <div className={styles.errorText}>프로필 로딩 실패: {error}</div>
            ) : profiles.length === 0 ? (
